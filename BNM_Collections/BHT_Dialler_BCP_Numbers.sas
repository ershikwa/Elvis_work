...
%macro GetLatestColl_ActvRate();
	proc sql;
		...
		select TABLE_NAME from PRDCStTables where TABLE_NAME like "Coll_ActvRate_&LastMonth.";
	quit;
	proc sql;
		create table Actvtemp2 as 
		select TABLE_NAME from PRDCStTables where TABLE_NAME like "Coll_ActvRate_&Last2Months.";
	quit;
...
%macro GetLatestColData();
	proc sql;
		...
		select TABLE_NAME from PRDCStTables where upcase(TABLE_NAME) like "COLDATA_&LastMonth.";
	quit;

	proc sql;
		create table Coltemp2 as 
		select TABLE_NAME from PRDCStTables where upcase(TABLE_NAME) like "COLDATA_&Last2Months.";
	quit;
...
%macro GetLatestProb2Honour();
	proc sql;
		...
		select TABLE_NAME from PRDCstTables where upcase(TABLE_NAME) like "KJM_PROB2HONOUR_SCORED_&LastMonth.";
	quit;

	proc sql;
		create table Probtemp2 as 
		select TABLE_NAME from PRDCstTables where upcase(TABLE_NAME) like "KJM_PROB2HONOUR_SCORED_&Last2Months.";
	quit;
...
%mend;
/*source???
Sphe Notes: What makes this table scoring.dbo.KJM_PROB2HONOUR_SCORED_?
 moved to PRD_ContactInfo (18/07/2021)
Answer: Prob2Honour_Extract_Score (01/10/2021) - In PRD_Collections_Strategy*/
...
proc sql stimer;
	...
	execute	
	(	
		IF object_id('Prd_ContactInfo.dbo.BHT_Dialler_1_DeD') is not null DROP TABLE Prd_ContactInfo.dbo.BHT_Dialler_1_DeD;
...
proc sql stimer;
	...
	execute	
	(	
		IF object_id('Prd_ContactInfo.dbo.BHT_Dialler_2') is not null DROP TABLE Prd_ContactInfo.dbo.BHT_Dialler_2;				
...
proc sql stimer;
	...
	execute	
	(	
		IF object_id('Prd_ContactInfo.dbo.BHT_Dialler_BCP_Numbers') is not null DROP TABLE Prd_ContactInfo.dbo.BHT_Dialler_BCP_Numbers;						
		create table Prd_ContactInfo.dbo.BHT_Dialler_BCP_Numbers
		with(distribution = hash(ClientNo), clustered columnstore index)
		as
...
proc sql stimer;
	...
	execute	
	(	
		update Prd_ContactInfo.dbo.BHT_Dialler_BCP_Numbers
...
%Upload_APS(Set = BHT_Dialler_BCP_Num_Hist_&TomorrowsMonth., Server = work, APS_ODBC = Prd_Cont, APS_DB = Prd_ContactInfo, Distribute = HASH(RunDate));
