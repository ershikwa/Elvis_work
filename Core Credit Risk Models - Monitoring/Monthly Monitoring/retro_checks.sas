...
/*select * from connection to odbc( */
	...
	avg(V655_2) as average_V655_2, std(V655_2) as std_v655_2, min(V655_2_finalscore) as min_V655_2_finalscore,
	max(V655_2_finalscore) as max_V655_2_finalscore, avg(V655_2_finalscore) as average_V655_2_finalscore,
	std(V655_2_finalscore) as std_V655_2_finalscore, count(V655_2) as V655_2_notnull, 
	...
	from &retro_dset
/*	where month >= &oneyearago and month <= &month*/
	group by month ;
/*	order by month desc); */
...
/*	select * from connection to odbc(*/
		select month, (case when comb_thin = . then . else comb_thin end) as comb_thin,
		count(case when comb_thin = . then 1 else 2 end) as vol_combthin
		from &retro_dset
/*		where month >= &oneyearago and month <= &month*/
...
/*		select * from connection to odbc(*/
		select month, (case when product = '' then '(blank)' else product end) as product,
		count(case when product = '' then 1 else 2 end) as vol_product
		from &retro_dset
/*		where month >= &oneyearago and month <= &month*/
...
/*		select * from connection to odbc(*/
		select month, (case when INSTITUTIONCODE = '' then '(blank)' else INSTITUTIONCODE end) as INSTITUTIONCODE,
		count(case when INSTITUTIONCODE = '' then 1 else 2 end) as vol_INSTITUTIONCODE
		from &retro_dset
/*		where month >= &oneyearago and month <= &month*/
...
/*		select * from connection to odbc(*/
		select month, (case when TypeCode = '' then '(blank)' else TypeCode end) as TypeCode,
		count(case when TypeCode = '' then 1 else 2 end) as vol_TypeCode
		from &retro_dset
/*		where month >= &oneyearago and month <= &month*/
...
/*libname amo "\\mpwsas64\Core_Credit_Risk_Model_Team\Amo";*/
/*data AMretro_checks_&m1 ;*/
/*	set amo.retro_checks_&m1 ;*/
/*run;*/
/**/
/*%Upload_APS(Set =AMretro_checks_&m1  , Server =Work, APS_ODBC = PRD_DDDa, APS_DB = PRD_DataDistillery_Data , distribute = HASH([month]));*/
...
proc sgplot data=retro_checks_&m ;
	vbar month / response=V655_2_notnull legendlabel= 'V655.2';
	vbar month / response=V655_2_zero legendlabel= 'V655.2 = 0';
	where rank=1;* compress(month) >= compress("&oneyearago") and compress(month) <= compress("&month") and rank=1;
	title "Volumes Over time where V655.2 is equal to 0 ";
	yaxis label='Volume';
run;
proc sgplot data=retro_checks_&m ;
	vbar month / response=V655_2_notnull legendlabel= 'V655.2';
	vbar month / response=V655_2_one legendlabel= 'V655 = 1';
	where rank=1;* compress(month) >= compress("&oneyearago") and compress(month) <= compress("&month") and rank=1;
	title "Volume Over time where V655.2 is equal to 1 ";
	yaxis label='Volume';
run;


proc sgplot data= retro_checks_&m ;
	vbar month / response= Vol_INSTITUTIONCODE group=INSTITUTIONCODE stat=sum missing datalabel;
...
proc sgplot data=retro_checks_&m ;
	vbar month / response=EMPLOYERSUBGROUPCODE_notnull legendlabel= 'EMPLOYERSUBGROUPCODE_non_null';
	vbar month / response=EMPLOYERSUBGROUPCODE_null legendlabel= 'EMPLOYERSUBGROUPCODE_null';
	where rank=1;* compress(month) >= compress("&oneyearago") and compress(month) <= compress("&month") and rank=1;
	title "Volume Over time where EMPLOYERSUBGROUPCODE is null and not null";
	yaxis label='Volume';
run;
/*proc sgplot data= retro_checks_&m ;
	vbar month / response= Vol_EMPLOYERSUBGROUPCODE group=EMPLOYERSUBGROUPCODE stat=sum missing datalabel;
...
proc sgplot data= retro_checks_&m ;
	vbar month / response= Vol_combthin group=comb_thin stat=sum missing datalabel;
...
proc sgplot data= retro_checks_&m ;
	vbar month / response= Vol_Product group=Product stat=sum missing datalabel;
...
proc sgplot data= retro_checks_&m ;
	vbar month / response= Vol_TypeCode group=TypeCode stat=sum missing datalabel;
