...
%macro getOutcomeData(inputdataset=,outcomeBase=);
	...
	data applicationbase_&month;
		set &inputdataset;
		finalriskscore_TU = (1- (TU_V570_prob))*1000;
...
/*TO remove special characters in baseloanids 	*/
	data final_model(keep = baseloanid);
		set applicationbase_&month;
		if input(baseloanid, best.) =. then delete;
	run;

	proc sql stimer;
		connect to odbc (dsn=mpwaps);
		select * from connection to odbc (
			drop table PRD_DataDistillery_data.dbo.final_model
		);
		disconnect from odbc;
	quit;
	%Upload_APS(Set =final_model , Server =Work, APS_ODBC = Prd_DDDa, APS_DB = PRD_DataDistillery_data , distribute = HASH([baseloanid]));
	...
	proc sql stimer;
		...
		select * from connection to odbc ( 
			...
			from PRD_DataDistillery_data.dbo.Final_model A 
			inner join PRD_DataDistillery_data.dbo.Disbursement_Info E
			on a.baseloanid = e.loanid
			...
			on a.baseloanid = right(d.uniqueid,9)
		) ;
		disconnect from odbc ;
	quit;
	...
	data &outcomeBase;
		set tu.DISBURSEDBASE4REBLT_&month(drop=weight);
	run;
%mend;
%getOutcomeData(inputdataset=tu.applicationbase_&month,outcomeBase=Disbursedbase4);
...
%macro calibration(Input=,Output=);
	...
	%Loopthroughcal(Dset=subset,seg=1,name = V561_seg,prob=CS_V560_Prob,Segmentation=Comp_seg,weight=weight);
	%Loopthroughcal(Dset=subset,seg=2,name = V561_Seg,prob=CS_V560_Prob,Segmentation=Comp_seg,weight=weight);
	%Loopthroughcal(Dset=subset,seg=3,name = V561_Seg,prob=CS_V560_Prob,Segmentation=Comp_seg,weight=weight);
	%Loopthroughcal(Dset=subset,seg=4,name = V561_Seg,prob=CS_V560_Prob,Segmentation=Comp_seg,weight=weight);
	%Loopthroughcal(Dset=subset,seg=5,name = V561_Seg,prob=CS_V560_Prob,Segmentation=Comp_seg,weight=weight);
	...
	proc sql;
		  ...
		  on a.Tu_seg = b.v561_Seg
	      left join parameters_V571_seg c
	      on a.Tu_seg = c.v571_Seg;
	quit;
	...
	proc sql;
	      ...
	      as select a.*,
	      1/(1+(V561/(1-V561))**(-1*(c.a))*exp(c.c)) as V562,
...
/*------------------------------------------------------------------*/	
/********************************************************************************************************************************************************/
/* 														Add Reject data into Disbursement data															*/
/********************************************************************************************************************************************************/
/*%macro addRejectsInferenceBase(RejectBase=,ApplicationBase=, DisburseBase=);*/
...
/*			where baseloanid in (select baseloanid from &DisburseBase where Principaldebt ne .);*/
/*	quit;*/
/*	proc sql;*/
/*		create table RejectApplications as*/
/*			select a.**/
/*			from applicationsbase a inner join &RejectBase b*/
/*			on a.baseloanid = b.tranappnumber ;*/
/*	quit;*/
/*	proc append base=NewAppbase data=RejectApplications  force; run;*/
/*%mend;*/
/*%addRejectsInferenceBase(RejectBase=rejects.application_rejects_&month, ApplicationBase=Applicationbase_&month, DisburseBase=DisbursedBase);*/
...
%macro AppendingSlopingTable (segment=,outdataset=);
	proc sql;
		create table _temptrendtable_ as
			...
			on  upcase(tranwrd(a.variable_name,"_S"," ")) = (b.Parameter);
	quit;
...
%macro combineContributionPerSegment();
	...
	%do seg = 1 %to 5;
		...
		%ContribCalc_Montoring(d&seg..wght_trans2, disb&seg,seg&seg._contrib);
...
%macro combineCorrelationPerSegment();
	...
	%do seg = 1 %to 5;
		proc sql; select Parameter into : modvars separated by ' ' from d&seg..wght_trans2 where  estimate ne . and upcase(parameter) ne 'INTERCEPT'; quit;
...
%macro plotconfidencebands_report(inputdata=, segment=) ;
	...
	quit;
	proc sql noprint;
		select effect into : descriptionvar separated by ' ' 
		from _estimates_&segment;
	quit;
...
proc sql;
	create table tu.v5_sloperate_reblt_&month. as
		...
		on  upcase(a.variable_name) = upcase(e.New_variable)
...
%CalcGinimetrics(segment =1,indataset=segment_1_gini, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);
%CalcGinimetrics(segment =2,indataset=segment_2_gini, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);
%CalcGinimetrics(segment =3,indataset=segment_3_gini, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);
%CalcGinimetrics(segment =4,indataset=segment_4_gini, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);
%CalcGinimetrics(segment =5,indataset=segment_5_gini, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);
%CalcGinimetrics(segment =0,indataset=Disbursedbase6, period=month,target=target, listofvargini=CS_V560_Prob Compuscan_Generic Tu_Generic TU_V570_prob V622 V623_Prob V562 V572 V635 V636 V645);

data tu.giniperseg_summary_reblt_&month;
	set  tu.giniperseg_summary_reblt_&month;
	rename CS_V560_Prob=V560_CS_V560_Prob  Compuscan_Generic=Comp_Generic_Score Tu_Generic=TU_Generic_Score TU_V570_prob=V570_TU_V570_prob V622=V622_Prob V562=V562_CS_V560_Prob V572=V572_TU_V570_prob V635=V635_Prob V636=V636_Prob;
run;
...
data basegini1(drop=type);
	...
	where type = 'TU_V570_prob';
	month = &month.;
	score_type = 'V570 TU Prob';
	applied_Model = 'V570 TU Model';
run;
...
proc sql;
	create table tu.currentmodel_bench_reblt_&month as
		...
		where score_type='TU_V570_prob') b
...
ods pdf body = "\\mpwsas64\Core_Credit_Risk_Models\V5\V5 Monitoring\TU Performance Monitoring\reports\V5.70\V570 monitoring pack &month..pdf"  ;

	ods pdf startpage = now;
	ods layout start;
	ods region y=0in x=0in width=4in height=4in;
	%VarCheckConfidenceBand1(Disbursedbase4, month, V637 , target , Principaldebt ,0,0,4,4, V570 TU Overall Model ) ;
	ods region y=0in x=4.5in width=4in height=4in;
	%VarCheckConfidenceBand1(Disbursedbase4, tu_seg, V637 , target , Principaldebt ,0,0,4,4, Segments ) ;
	ods region y=4.5in x=0in width=4in height=4in;
	%VarCheckConfidenceBand1(Disbursedbase4, Decile_b, V637 , target , Principaldebt ,0,0,4,4, Overall Decile ) ;
	ods region y=4.5in x=4.5in width=4in height=4in;
	%VarCheckConfidenceBand1(Disbursedbase4, RG6T, V637 , target , Principaldebt ,0,0,4,4, Risk Group ) ;
	ods layout end ;
	ods pdf startpage = now;
	ods layout start;
	ods region y=0in x=0in width=4in height=4in;
	%VarCheckConfidenceBand1(Disbursedbase4, INSTITUTIONCODE , V637 , target , Principaldebt ,0,0,4,4, INSTITUTION CODE) ;
	ods region y=0in x=4.5in width=4in height=4in;
	...
	ods region y=4.5in x=0in width=4in height=4in;
	%VarCheckConfidenceBand1(BNKABL, month, V637 , target , Principaldebt ,0,0,4,4, BNKABL ) ;
	ods region y=4.5in x=4.5in width=4in height=4in;
	%VarCheckConfidenceBand1(BNKINV, month, V637 , target , Principaldebt ,0,0,4,4, BNKINV ) ;
	ods layout end ;
	ods pdf startpage = now;
	ods layout start;
	ods region y=0in x=0in width=4in height=4in;
