...
libname comp "\\neptune\sasa$\Tshegofatso Freddy Sehlapelo\Business Case"; /* Save the final summary table here */
...
data _null_;
	 ...
	 call symput("month_end", put(intnx("month", today(),-1,'end'),date9.));
	 call symput("start", put(intnx("month", today(),-13,'begin'),date9.));
	 call symput("actual_date", put(intnx("month", today(),-9,'end'),date9.));
	 call symput('lstmnth',cats("'",put(intnx('month',today(),-1),yymmddd10.),"'"));
run;
...
proc sql stimer; 
	...
	select * from connection to odbc (
	    drop table scoring.dbo.TF_final_model_Apps
	);
	disconnect from odbc;
quit;

data TF_final_model_Apps;
	set model_data (keep = tranappnumber);
run;
%Upload_APS(Set = TF_final_model_Apps , Server = Work, APS_ODBC = Scoring, APS_DB = Scoring , distribute = HASH([tranappnumber]));
...
proc sql stimer;        
        ...
        select * from connection to odbc (
            select A.tranappnumber,
                    ...
                    B.FirstDueMonth,
                    E.FirstDueDate,
                    cast(d.LNG_SCR as int) as TU_Score, Sample_Ind = 'DISBURSED'
            from scoring.dbo.TF_final_model_Apps A
...
%scoreinputdata(inputdataset=FinalModel_Data,numberofsegment=5,outputdataset=Rebuild_model,
				path=\\neptune\sasa$\V5\Application Scorecard\V6 Monitoring\V570 Rebuild Models\TU_Rebuild_&month);
...
%macro MonitorRebuilt(in_data, segnum, lst3mnths, outputdataset);
	%do p=1 %to &segnum;
		%let path = \\neptune\sasa$\V5\Application Scorecard\V6 Monitoring\V570 TU Refit;
		
		libname data3M_&p. "\\neptune\sasa$\V5\Application Scorecard\V6 Monitoring\V570 TU Refit\TU_Refit_&lst3mnths\segment&p.";
		libname data "\\neptune\sasa$\V5\V5 Monitoring\Performance Monitoring\Transunion Monitoring\datasets";
		...
		libname iv "\\neptune\sasa$\V5\V5 Monitoring\TU Performance Monitoring\V570 TU Refit\Seg&p.\iv";
		...
		proc sql;
			...
			select Capri_uniqueid, &&segment_&p._list2., month, seg, tranappnumber
			from build1
			where seg = &p.;
		quit;

		data final_model_data_;
		...
		data refit_scored_&p.;
		    set final_model_data_;  
			probability = Final_score;
		run;
	%end;
	data &outputdataset;
		set refit_scored_1 refit_scored_2 refit_scored_3 refit_scored_4 refit_scored_5;
	run;
%mend;
%MonitorRebuilt(FinalModel_Data, 5, &month, Refit_model);

data Refit_scorecard (keep=tranappnumber V5_Refit);
	set Refit_model;
	V5_Refit = probability;
run;

proc sort data=Refit_scorecard nodupkey; 
by tranappnumber;
run;
...
data Applicationbase_&Assump_month;
	set tu.APPLICATIONBASE_&Assump_month;
	month = substr(compress(ApplicationDate, '-'), 1, 6);
...
proc sql stimer;        
        ...
        select * from connection to odbc (
            ...
            on a.tranappnumber = B.loanid
            left join PRD_PRESS.[capri].[CAPRI_BUR_PROFILE_TRANSUNION_PLSCORECARD] d
            /*on b.loanid = right(d.uniqueid,9)*/
			on a.tranappnumber = right(d.uniqueid,9)
        ) ;
        disconnect from odbc ;
quit;
...
Data Create_RG;
    ...
    finalRiskScore = 1000-(V6_Prob3*1000);
	thinfileindicator = tu_thin;
...
%runme(Input=Rebuild, prob=V5_Rebuild, expected=tu_prob, MName=Rebuild, Thin=tu_thin);
proc append base = all_summary1 data = summary1 force; run;
proc append base = all_summary2 data = summary2 force; run;

%runme(Input=Refit, Prob=V5_Refit, expected=tu_prob, MName=Refit, Thin=tu_thin);
proc append base = all_summary1 data = summary1 force; run;
proc append base = all_summary2 data = summary2 force; run;

%runme(Input=V622, Prob=V6_Prob3, expected=tu_prob, MName=V622, Thin=tu_thin);
proc append base = all_summary1 data = summary1 force; run;
proc append base = all_summary2 data = summary2 force; run;

%runme(Input=V635, Prob=V635, expected=tu_prob, MName=V635, Thin=tu_thin);
proc append base = all_summary1 data = summary1; run;
proc append base = all_summary2 data = summary2; run;

%runme(Input=V636, Prob=V636, expected=tu_prob, MName=V636, Thin=tu_thin);
proc append base = all_summary1 data = summary1; run;
proc append base = all_summary2 data = summary2; run;

%runme(Input=V570, Prob=tu_prob, expected=tu_prob, MName=V570, Thin=tu_thin);
proc append base = all_summary1 data = summary1; run;
proc append base = all_summary2 data = summary2; run;

/* --------------------------------------------------------- */
data Disbursedbase_&month;
set tu.disbursedbase4reblt_&month;
seg = tu_seg;
run;
%scoreinputdata(inputdataset=Disbursedbase_&month,numberofsegment=5,outputdataset=Rebuild_model,
				path=\\neptune\sasa$\V5\Application Scorecard\V6 Monitoring\V570 Rebuild Models\TU_Rebuild_&month);
...
%MonitorRebuilt(Disbursedbase_&month, 5, &month, Refit_model);
...
%Ginis(Prob=V6_Prob3, Input=Disbursedbase_&month, MName=V622, target=target);
%Ginis(Prob=V635, Input=Disbursedbase_&month, MName=V635, target=target);
%Ginis(Prob=V636, Input=Disbursedbase_&month, MName=V636, target=target);
%Ginis(Prob=tu_prob, Input=Disbursedbase_&month, MName=V570, target=target);
...
data relative_change;
	set transposed;
	rc_Rebuild = (V570-Rebuild)/Rebuild;
	rc_Refit = (V570-Refit)/Refit;
	rc_V570 = (V570-V570)/V570;
	rc_V622 = (V570-V622)/V622;
	rc_V635 = (V570-V635)/V635;
	rc_V636 = (V570-V636)/V636;
run;

proc transpose data=relative_change out=relative_change2; 
	id _NAME_ ; 
	var Rebuild Refit V570 V622 V635 V636 rc_Rebuild rc_Refit rc_V570 rc_V622 rc_V635 rc_V636;
run;
...
%format(in=relative_change2, mod=V570);
%format(in=relative_change2, mod=V622);
%format(in=relative_change2, mod=V635);
%format(in=relative_change2, mod=V636);
...
%format(in=relative_change2, mod=rc_V570);
%format(in=relative_change2, mod=rc_V622);
%format(in=relative_change2, mod=rc_V635);
%format(in=relative_change2, mod=rc_V636);
...
proc delete data = cred_scr.TU_Business_Case; run;
proc sql;
	create table  cred_scr.TU_Business_Case (BULKLOAD=YES) as
	    select  distinct * 
		from Comp.business_case_&month.;
quit;

proc delete data =cred_scr.TU_bc_Tables; run;
proc sql;
	create table  cred_scr.TU_bc_Tables (BULKLOAD=YES) as
	    select  distinct * 
