...
Libname Calib '\\mpwsas64\Core_Credit_Risk_Model_Team\Scorecard\V655.2\Calibration';
libname ICB "\\mpwsas64\Core_Credit_Risk_Model_Team\Scorecard\V655.2\Data";
...
/*combine UBANK and all AB subgroup codes into AB since it is now 1 employer*/
proc sql;
	create table disbursement_info as
...
data month_12_volume_v2;
	...
	If ( volume_last_12_month >= 0 and volume_last_12_month <= 7156) or volume_last_12_month in (-1)  then volume_last_12_month_B = "B1: volume_last_12_month  <= 7156 or volume_last_12_month in (-1)";
	Else If (volume_last_12_month > 7156)  then volume_last_12_month_B = "B2: volume_last_12_month  > 7156";
...
proc sql;
    create table month_12_volume3 as
        ...
        left join icb.subsector_names b
        on upcase(a.subsector) = upcase(b.label)
        left join icb._ESTIMATE1_  c
...
data V6_calibration_volume_subsector;
	...
	Format tValue_B $500. ; 
	If tValue  <= -4.409312325 then tValue_B = "B1: tValue  <= -4.409312325";
	Else If tValue  > -4.409312325 or tValue in (.) then tValue_B = "B2: tValue  > -4.409312325 or tValue in (.)";
...
data EmployerVolumeIndex;
	...
	if upcase(employersubgroupcode) ='PREAPPROVEDOFFER' then SubSectorIndexGroup = '2';
run;

proc sql;
	create table EmployerVolumeIndex_V655 as
...
%Upload_APS(Set = EmployerVolumeIndex_V655 , Server =Work, APS_ODBC = PRD_DaDi, APS_DB = PRD_DataDistillery , distribute = HASH([employersubgroupcode]));
