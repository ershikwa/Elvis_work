...
data _null_;
	TableName = "Prd_DataDistillery_data.dbo.Disbursement_Info";
	call symput('TableName', TableName);
	call symput('TableName2', compress("'"||TableName||"'"));
run;
%PUT &TableName;
%PUT &TableName2;
...
IF OBJECT_ID('prd_DataDistillery_data.dbo.Disbursement_Info_Backup', 'U') IS NOT NULL DROP TABLE prd_DataDistillery_data.dbo.Disbursement_Info_Backup;
...
, 'Gazelle' as TblRef from ARC_LOANQ_2028.dbo.Loanquotation where Status='DIS' /*--Gazelle Apps*/
union all
...
, 'Gazelle-Loan' as TblRef from ARC_LOANQ_2028.dbo.LoanquotationAccount where isnull(Capital,0)>0 /*--Gazelle loan*/
union all
select LoanID, AccountNumber as LoanReference, create_date
, 'Gazelle-Card' as TblRef from ARC_LOANQ_2028.dbo.LOANQUOTATIONCARDDETAILS /*--Gazelle card*/
union all
...
union all
select loanid, ltrim(rtrim(LoanReference)) as LoanReference, 'Loan' as TblRef, 'Gazelle Loan' as TblRef0 from ARC_LOANQ_2028.dbo.LoanQuotationAccount (nolock) 
union all
select loanid, AccountNumber as LoanReference, 'Card' as TblRef, 'Gazelle Card' as TblRef0 from ARC_LOANQ_2028.dbo.LOANQUOTATIONCARDDETAILS
...
select distinct * from
(
select loanid, WageType, row_number() over( partition by loanid order by create_date desc) as RN from ARC_LOANQ_2028..LOANQUOTATIONEMPLOYMENTDETAILS (nolock) where ltrim(rtrim(isnull(WageType,''))) not in ('') /*--Gazelle (Card+loan)*/
    union all
...
/*select top 10 * from ARC_LOANQ_2028.dbo.LOANQUOTATIONCARDDETAILS (nolock)
-- Loanid, accountnumber
select loanid, count(*) as cnt from ARC_LOANQ_2028.dbo.LOANQUOTATIONCARDDETAILS (nolock)
group by  loanid
having count(*)>1*/
...
IF OBJECT_ID(&TableName2, 'U') IS NOT NULL DROP TABLE &TableName;
...
IF OBJECT_ID('prd_DataDistillery.dbo.Disbursement_Info', 'U') IS NOT NULL DROP TABLE prd_DataDistillery.dbo.Disbursement_Info;
...
IF OBJECT_ID('prd_DataDistillery_data.dbo.Disbursement_Info0', 'U') IS NOT NULL DROP TABLE prd_DataDistillery_data.dbo.Disbursement_Info0;
