...
proc sql;
 create table vols as
 	select distinct appmonth, volume_TuSeg1,volume_TuSeg2,volume_TuSeg3,volume_TuSeg4,volume_TuSeg5
 	from  tu_checks_&appmonth;
quit;
...
proc sql;
 create table tu_thin as
 	select distinct appmonth, TU_thin0,TU_thin1,TU_thinNull
 	from  tu_checks_&appmonth;
quit;

proc transpose data = tu_thin name=seg label=seg out= outdata;
by appmonth;
run;
...
proc sgplot data= tu_thin pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
...
proc sql;
 create table TU_V580 as
 	select distinct appmonth, TU_V580_prob_non_null,TU_V580_prob_null
 	from  tu_checks_&appmonth;
quit;

proc transpose data = TU_V580 name=seg label=seg out= outdata;
by appmonth;
run;
...
proc sgplot data= TU_V580 pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
...
proc sql;
 create table WAGEFREQ as
 	select distinct appmonth, WAGEFREQUENCYCODE001,WAGEFREQUENCYCODE002,WAGEFREQUENCYCODE003, WAGEFREQUENCYCODEnull
 	from  tu_checks_&appmonth;
quit;

proc transpose data = WAGEFREQ name=seg label=seg out= outdata;
by appmonth;
run;
...
proc sgplot data= WAGEFREQ pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
	title "Volumes Over time per WAGEFREQUENCYCODE";
	yaxis label='Volume';
run;
...
proc sql;
 create table PERSAL as
 	select distinct appmonth, PERSALINDICATOR0,PERSALINDICATOR1,PERSALINDICATORnull
 	from  tu_checks_&appmonth;
quit;

proc transpose data = PERSAL name=seg label=seg out= outdata;
by appmonth;
run;
...
proc sgplot data= PERSAL pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
	title "Volumes Over time per PERSALINDICATOR";
	yaxis label='Volume';
run;
...
proc sql;
 create table Repeat as
 	select distinct appmonth, Repeat1,Repeat0,Repeatnull
 	from  tu_checks_&appmonth;
quit;

proc transpose data = Repeat name=seg label=seg out= outdata;
by appmonth;
run;
...
proc sgplot data= Repeat pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
	title "Volumes Over time per Repeat";
	yaxis label='Volume';
run;
...
proc sql;
 create table EMPLOY as
 	select distinct appmonth, EMPLOYERSUBGROUPCODE_notnull, EMPLOYERSUBGROUPCODE_null
 	from  tu_checks_&appmonth;
quit;
...
proc sgplot data= EMPLOY pctlevel=group;
	vbar appmonth/response=COL1 group=seg stat=pct missing datalabel;
	title "Volumes Over time per EMPLOYERSUBGROUPCODE";
	yaxis label='Volume';
run;


proc sgplot data= tu_checks_&appmonth pctlevel=group;
	vbar appmonth / response= Vol_EMPLOYERSUBGROUPCODE group=EMPLOYERSUBGROUPCODE stat=pct missing datalabel;
...
proc sgplot data= tu_checks_&appmonth pctlevel=group;
	vbar appmonth / response= Vol_TypeCode group=TypeCode stat=pct missing datalabel;
...
proc sgplot data= tu_checks_&appmonth pctlevel=group;
	vbar appmonth / response= vol_cstAge group=cstAge stat=pct missing datalabel;
