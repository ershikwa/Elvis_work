...
%macro dailyMonitoring();
	...
	%if &no_data ne 2 %then %do;
		...
		%GetData(table=DEV_DataDistillery_Credit.dbo.V6DailyData, outDataset=vic.rawdata5, dsn=DEV_DDCr);
...
/**/
		...
		%applyCS_TU(inDataset=vic.rawdata5, outDataset=TBL_Scored);
		%include "\\mpwsas64\Core_Credit_Risk_Model_Team\GitLab_Local_Repo\Apply_V575.sas";
		...
		data all_scores3;
			...
			if  round(V660_prod ,0.0001) - round(input(V650,best16.),0.0001) = 0 then V660_Match = 1;
		    else V660_Match =0;
			...
			else if scorecardversion = 'V667' then V6_match = V667_match;
			if scorecardversion in ('V655','V667') then V6_match_IA = V658_match;
