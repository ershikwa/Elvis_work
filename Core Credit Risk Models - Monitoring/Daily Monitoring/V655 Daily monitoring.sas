...
%macro dailyMonitoring();
	...
	%if &no_data ne 2 %then %do;
		...
		/*Get cleaned data*/
		%include "\\MPWSAS64\Core_Credit_Risk_Model_Team\GitLab_Local_Repo\Daily Monitoring\GetData_V655.sas";
		...
		%include "\\mpwsas65\Process_Automation\Codes\Git_Repositories\Core Credit Risk Models - Monitoring\Macros\ApplyV655.sas";
		...
		proc sql;
			...
			e.V650, e.V651, e.V652, e.V653, e.V654, e.V655, a.*
			from rawdata6 a
		...
		data all_scores2;
			set all_scores1;
			if maxscorecardversion ne 'V655' then do;
			...
			else if maxscorecardversion = 'V655' then do;
				if Min(Comp,TU_Thin) = 0 then do;
...
/*				sum(CAPRI_APPLICANT)/count(CAPRI_APPLICANT) as APPLICANT_TABLE format=percent9.2,*/
		...
		proc sort data=batchtes.V622_intables;
		by applicationdate;
		run;
		...
		%include "\\MPWSAS64\Core_Credit_Risk_Model_Team\GitLab_Local_Repo\Daily Monitoring\Report_inclWeekends.sas";
		%include "\\MPWSAS64\Core_Credit_Risk_Model_Team\GitLab_Local_Repo\Daily Monitoring\Report_exclWeekends.sas";
/*		%include "\\mpwsas65\Process_Automation\Codes\Git_Repositories\Core Credit Risk Models - Monitoring\Macros\Report_Dashboard.sas";*/
