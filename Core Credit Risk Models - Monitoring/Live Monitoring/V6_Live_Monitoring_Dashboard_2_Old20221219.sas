...
libname scoring3 odbc dsn=DEV_DDRe schema='dbo' preserve_tab_names=yes connection=unique direct_sql=yes;
...
libname scoring odbc dsn=DEV_DDCr schema='dbo' preserve_tab_names=yes connection=unique direct_sql=yes;
    ...
    create table scoring.V6_live_scorecardversion like V6_live_scorecardversion;

    insert into scoring.V6_live_scorecardversion (bulkload=Yes)
    select *
    from V6_live_scorecardversion;
quit;
...
proc sql;
	...
	select * 
	from All_scores a
	left join scoring.V6_Live_Decline_Codes b 
	on a.declinecode = b.declinecode
	left join scoring.V6_TypeCode_lookup c
...
libname scoring odbc dsn=DEV_DDCr schema='dbo' preserve_tab_names=yes connection=unique direct_sql=yes;
	...
	drop table scoring.V622_Live_All_scores_v622;

	create table scoring.V622_Live_All_scores_v622 like LiveDash.All_scores_v622_&todaysDate.;

	insert into scoring.V622_Live_All_scores_v622 (Bulkload = Yes)
	select distinct  *
